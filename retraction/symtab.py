...
class SymTab:
    ...
    def add_entry(
        ...
        self.table.append(Entry(name, entry_type, ast_node))
    ...
    def set_entry_node(
        ...
        """
        Set the AST node for an entry in the symbol table.
        This is needed because the AST node may not be known at the time of entry creation,
        since the node and entry refer to each other.
        """
        self.table[index].ast_node = ast_node

    def find(self, name: str) -> Entry:
        if name not in self.lookup:
            if self.parent is not None:
                return self.parent.find(name)
            raise SyntaxError(f"Symbol {name} not found")
        return self.table[self.lookup[name]]
